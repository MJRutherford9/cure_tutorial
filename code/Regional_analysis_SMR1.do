...
use ".\data\combined_dataset_regional.dta", clear
sort dataset id
...
stset OS_mm if dataset==0, failure(dead) id(id) scale(12) 
stexpect3 using "https://pclambert.net/data/popmort.dta" if dataset==0, agediag(age_accurate) datediag(datediag) pmother(sex) pmage(_age) pmyear(_year) every(0.05) maxt(50) pmmaxyear(1995) pmmaxage(99)
...
merge m:1 _year sex _age using "https://pclambert.net/data/popmort.dta", keep(match master)

* Fit the 24 month models first
* for the 24 month models, run stexpect on the 24 month dataset
stset OS_mm if dataset==1, failure(dead) id(id) scale(12) 
stexpect3 using "https://pclambert.net/data/popmort.dta" if dataset==1, agediag(age_accurate) datediag(datediag) pmother(sex) pmage(_age) pmyear(_year) every(0.05) maxt(50) pmmaxyear(1995) pmmaxage(99)
...
stpm2, df(4) scale(h) bknots(0.01 15) bhazard(rate) cure
...
**HERE BYPASSING FRAMES LIMIT OF 100

forvalues j=1/11 {
	local start_loop=50*(`j'-1)
	local S_temp_list
	forvalues t=`start_loop'(1)`=`start_loop'+49' {	
			local S_temp_list `S_temp_list' S_temp_exp`t'
	}

	stexpectind `S_temp_list' using "https://pclambert.net/data/popmort" if dataset==1, times(`=`start_loop'/10'(0.1)`=4.9+(`start_loop'/10)') ///
					agediag(age_accurate) ///
					datediag(dx) ///
					pmother(sex) pmmaxyear(1995)
					
}

	tempfile tmppoport
	preserve
		use "https://pclambert.net/data/popmort", clear
		rename rate rate2
	save `tmppoport', replace
	restore
...
forvalues t=0(1)501 {	
	...
	capture drop _merge 
	capture drop rate2
	replace _age=floor(age_accurate+`=`t'/10')
	replace _age=99 if _age>=100
	replace _year=floor(min(year(dx+365.25*`=`t'/10'),1995))
	
	merge m:1 _age _year sex using `tmppoport', keepus(rate2) keep(match master)			
	replace rate2=. if dataset!=1	
	
	*survival
	predict ssurv`t', timevar(t) surv
	gen ssurv_overall`t'= ssurv`t'*S_temp_exp`t'
	
	*hazard
	predict hazard`t', timevar(t) hazard
	gen haz_overall`t'= ssurv_overall`t'*(hazard`t' + rate2)
	...
	capture drop ssurv`t'  ssurv_overall`t'
	capture drop hazard`t' haz_overall`t'
	capture drop rate2
...
estat ic
* standsurv rmst doesn't run for this model: max iterations of Dormand Prince method exceeded
capture standsurv if dataset==1, rmst ci             ///
           timevar(t50)        ///
           at1(.) atvars(rmst_m9) ///
            expsurv(using("https://pclambert.net/data/popmort.dta")  ///
...
estat ic
* standsurv rmst may not run for this model: max iterations from Dormand Prince method may be exceeded. Can integrate instead to estimate RMST
capture standsurv if dataset==1, rmst ci             ///
           timevar(t50)        ///
           at1(.) atvars(rmst_m10) ///
            expsurv(using("https://pclambert.net/data/popmort.dta")  ///
...
*stpm2 rcsage*, tvc(rcsage*) dftvc(3) df(4) scale(h) bknots(0.01 15) bhazard(rate) cure
stpm2 rcsage*, tvc(rcsage*) dftvc(2) df(4) scale(h) bknots(0.01 15) bhazard(rate) cure
...
stset OS_mm if dataset==2, failure(dead) id(id) scale(12) 
stexpect3 using "https://pclambert.net/data/popmort.dta" if dataset==2, agediag(age_accurate) datediag(datediag) pmother(sex) pmage(_age) pmyear(_year) every(0.05) maxt(50) pmmaxyear(1995) pmmaxage(99)
...
forvalues j=1/11 {
	...
	stexpectind `S_temp_list' using "https://pclambert.net/data/popmort" if dataset==2, times(`=`start_loop'/10'(0.1)`=4.9+(`start_loop'/10)') ///
					agediag(age_accurate) ///
					datediag(dx) ///
					pmother(sex) pmmaxyear(1995)
...
save ".\data\regional_models_for_plots_SMR1.dta", replace
