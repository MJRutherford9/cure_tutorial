*** Distant Model Plots with SMR=2.5 ***

* See "Distant_analysis_SMR25.do" for fitting of models
...
*** Model 7 did not converge, remove below
...
sts graph, ci plotopts(lwidth(medium) lpattern(dash))  /// 
		...
		7 "Non-mixture cure model, df3, 15y BK" 8 "Non-mixture cure model, df5, 15y BK" 9 "Mixture cure model (Weibull), age" ///
		10 "Non-mixture cure model, df3, 5y BK, age" 11 "Non-mixture cure model, df5, 5y BK, age" 12 "Non-mixture cure model, df3, 15y BK, age" ///
		13 "Non-mixture cure model, df5, 15y BK, age" 14 "FPM non-cure, df4" ///
		15 "Expected survival background population") size(vsmall)) xscale(range(0 50)) xlabel(0(5)50)) ///
        ...
        graphregion(color(white)) plotregion(fcolor(white)) name(Distant_Surv50_SMR25_24m,replace)
...
sts graph, hazard ci plotopts(lwidth(medium) lpattern(dash)) /// 
		...
		xtitle(Time since randomisation (years)) xtitle(, size(small)) xlabel(, labsize(small)) /// 
        graphregion(color(white)) plotregion(fcolor(white)) name(Distant_Haz50_SMR25_24m,replace)
...
sts graph, ci plotopts(lwidth(medium) lpattern(dash))  /// 
		...
		10 "Non-mixture cure model, df3, 5y BK, age" 11 "Non-mixture cure model, df5, 5y BK, age" 12 "Non-mixture cure model, df3, 15y BK, age" ///
		13 "Non-mixture cure model, df5, 15y BK, age" 14 "FPM non-cure, df4" ///
		15 "Expected survival background population") size(vsmall)) xscale(range(0 20)) xlabel(0(2)20)) ///
		legend(pos(2) ring(0) cols(2)) title(, size(zero) nobox) ytitle(Proportion surviving) ytitle(, size(small)) ///
        ylabel(#8, labsize(small) angle(horizontal) format(%9.2f) nogrid) yscale(range(0 1.0)) ///
		xtitle(Time since randomisation (years)) xtitle(, size(small)) xlabel(, labsize(small)) /// 
        graphregion(color(white)) plotregion(fcolor(white)) name(Distant_Surv20_SMR25_24m,replace)
...
sts graph, hazard ci plotopts(lwidth(medium) lpattern(dash)) /// 
        ...
        ylabel(#10, labsize(small) angle(horizontal) format(%9.2f) nogrid) yscale(range(0 1.0)) ///
		xtitle(Time since randomisation (years)) xtitle(, size(small)) xlabel(, labsize(small)) /// 
        graphregion(color(white)) plotregion(fcolor(white)) name(Distant_Haz20_SMR25_24m,replace)
...
sts graph, ci plotopts(lwidth(medium) lpattern(dash))  /// 
        ...
        ylabel(#8, labsize(small) angle(horizontal) format(%9.2f) nogrid) yscale(range(0 1.0)) ///
		xtitle(Time since randomisation (years)) xtitle(, size(small)) xlabel(, labsize(small)) /// 
        graphregion(color(white)) plotregion(fcolor(white)) name(Distant_Surv50_SMR25_48m,replace)
...
sts graph, hazard ci plotopts(lwidth(medium) lpattern(dash)) /// 
        ...
        ylabel(#10, labsize(small) angle(horizontal) format(%9.2f) nogrid) yscale(range(0 1.0)) ///
		xtitle(Time since randomisation (years)) xtitle(, size(small)) xlabel(, labsize(small)) /// 
        graphregion(color(white)) plotregion(fcolor(white)) name(Distant_Haz50_SMR25_48m,replace)
...
sts graph, ci plotopts(lwidth(medium) lpattern(dash))  /// 
        ...
        ylabel(#8, labsize(small) angle(horizontal) format(%9.2f) nogrid) yscale(range(0 1.0)) ///
		xtitle(Time since randomisation (years)) xtitle(, size(small)) xlabel(, labsize(small)) /// 
        graphregion(color(white)) plotregion(fcolor(white)) name(Distant_Surv20_SMR25_48m,replace)
...
sts graph, hazard ci plotopts(lwidth(medium) lpattern(dash)) /// 
        ...
        ylabel(#10, labsize(small) angle(horizontal) format(%9.2f) nogrid) yscale(range(0 1.0)) ///
		xtitle(Time since randomisation (years)) xtitle(, size(small)) xlabel(, labsize(small)) /// 
        graphregion(color(white)) plotregion(fcolor(white)) name(Distant_Haz20_SMR25_48m,replace)
