*** Regional Analysis with SMR=2.5 ***

* See "Deriving_regional_dataset.do" for deriving the datasets
...
* All these are with an SMR of 2.5
...
* now merge with life table with SMR applied. 
* Note, I'm doing this by multiplying the rates in the life tables by the SMR (i.e. by 2.5). 
* below merge gives everyone across all the datasets a matched expected mortality rate
merge m:1 _year sex _age using ".\data\popmort_SMR25.dta", keep(match master)

* Fit the 24 month models first
* for the 24 month models, run stexpect on the 24 month dataset
stset OS_mm if dataset==1, failure(dead) id(id) scale(12) 
stexpect3 using ".\data\popmort_SMR25.dta" if dataset==1, agediag(age_accurate) datediag(datediag) pmother(sex) pmage(_age) pmyear(_year) every(0.05) maxt(50) pmmaxyear(1995) pmmaxage(99)
...
capture standsurv if dataset==1, rmst ci             ///
           timevar(t50)        ///
           at1(.) atvars(rmst_m3) ///
            expsurv(using(".\data\popmort_SMR25.dta")  ///
...
foreach id of local idlevs {
	...
	quietly { 
		stexpect3 using ".\data\popmort_SMR25.dta" if id==`id' & dataset==1, agediag(age_accurate)  ///
...
stset OS_mm if dataset==2, failure(dead) id(id) scale(12) 
stexpect3 using ".\data\popmort_SMR25.dta" if dataset==2, agediag(age_accurate) datediag(datediag) pmother(sex) pmage(_age) pmyear(_year) every(0.05) maxt(50) pmmaxyear(1995) pmmaxage(99)
...
foreach id of local idlevs {
	capture drop t_exp expsurv exphaz
	quietly { 
		stexpect3 using ".\data\popmort_SMR25.dta" if id==`id' & dataset==2, agediag(age_accurate)  ///
		   datediag(datediag) pmother(sex) pmage(_age)                    ///
		   pmyear(_year) every(0.1) maxt(50)                          ///
		   pmmaxyear(1995) pmmaxage(99)
...
save ".\data\regional_models_for_plots_SMR25.dta", replace
